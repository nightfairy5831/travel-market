...
// import CompanionSubProfile from "./CompanionSubProfile";

// export default function CompanionProfilePage({ email, user, profile }) {
//   // console.log(profile)
...
//   const { data: existingCompanion, isLoading: isLoadingCompanion } = useQuery({
//     queryKey: ["companion", user?.id],
//     queryFn: async () => {
//       if (!user?.id) {
//         console.warn("⚠️ No user.id, skipping companion query.");
//         return null;
//       }
...
//         return data || null;
//       } catch (err) {
//         console.error("💥 Error in companion query:", err);
//         throw err;
//       }
//     },
//     enabled: !!user?.id,
...
//       console.log("✅ Existing companion found:", existingCompanion);
//       setFormData((prev) => ({
//         ...prev,
//         basics: {
//           firstName: existingCompanion.first_name || "",
//           lastName: existingCompanion.last_name || "",
//           email: existingCompanion.email || email || "",
//           phone: existingCompanion.phone || "",
//           language: Array.isArray(existingCompanion.languages)
//             ? existingCompanion.languages
//             : existingCompanion.languages
//             ? [existingCompanion.languages]
//             : [],
//           gender: existingCompanion.gender || "",
//           specialNeeds: existingCompanion.special_needs || "",
//           phoneVerified: existingCompanion.is_phone_verified || false,
//           emailVerified: existingCompanion.is_email_verified || false,
//         },
//       }));

//       if (existingCompanion.profile_photo_url) {
//         setProfilePhoto({ url: existingCompanion.profile_photo_url });
//       }
//     } else if (email) {
...
//   const isEditing = !!existingCompanion;
...
//               <Heading
//                 title={
//                   isEditing ? "Update Companion Profile" : "Companion Profile"
//                 }
...
export default function CompanionProfilePage({ email, user, profile }) {
    ...
    if (existingCompanion) {
      ...
      setFormData((prev) => ({
        ...
        basics: {
          ...
          is_kyc_approved: existingCompanion.is_kyc_approved || false,
          dob: existingCompanion.dob || "",
          preferred_airports: Array.isArray(existingCompanion.preferred_airports)
            ? existingCompanion.preferred_airports
            : existingCompanion.preferred_airports
            ? [existingCompanion.preferred_airports]
            : [],
          service_types: Array.isArray(existingCompanion.service_types)
            ? existingCompanion.service_types
            : existingCompanion.service_types
            ? [existingCompanion.service_types]
            : [],
          skill_certificates: Array.isArray(existingCompanion.skill_certificates)
            ? existingCompanion.skill_certificates
            : existingCompanion.skill_certificates
            ? [existingCompanion.skill_certificates]
            : [],
          short_bio: existingCompanion.short_bio || "",
        },
      }));
  ...
  const validateBasics = () => {
    ...
    // Validate new fields if needed
    if (!basics.dob?.trim()) errs.dob = "Date of birth is required";
    if (!basics.preferred_airports?.length) errs.preferred_airports = "Select at least one preferred airport";
    if (!basics.service_types?.length) errs.service_types = "Select at least one service type";
    if (!basics.short_bio?.trim()) errs.short_bio = "Short bio is required";
  ...
  const saveTravellerMutation = useMutation({
    ...
    onSuccess: (data) => {
      setIsLoading(false);
      queryClient.invalidateQueries(["companion", user?.id]);
      showSuccess("Profile updated successfully");
      // router.push("/dashboard");
    },
    onError: (error) => {
